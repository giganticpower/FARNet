...
class Ctw1500Text(TextDataset):
    ...
    def __init__(self, data_root, is_training=True, transform=None, ignore_list=None):
        ...
        self.annotation_root = os.path.join(data_root, 'train' if is_training else 'test', "text_label_curve" if is_training else 'text_label_circum' )
...
if __name__ == '__main__':
    from util.augmentation import Augmentation1, Augmentation
    ...
    for idx in range(0,5):
        ...
        print(idx, img.shape)
        img = cv2.imread('/data/chh/DRRG-master/data/ctw1500/show/text_image/1410.jpg')

        # tr = cv2.cvtColor(tr_mask * 255, cv2.COLOR_GRAY2BGR)
        # tcl = cv2.cvtColor(tcl_mask[:, :, 0] * 255, cv2.COLOR_GRAY2BGR)
        # im_vis = np.concatenate([img, tr, tcl], axis=1)
        # cv2.imwrite('wave_ctw_{}.jpg'.format(idx), im_vis)
        ...
        for bbox_idx in range(1, ret):
            ...
            boxes = bbox_transfor_inv(radius_map, sin_map, cos_map, text_map, wclip=(2, 16))
            # nms
            ...
            if boxes.shape[0] > 1:
                ...
                scale1 = random.randint(0, 255)
                scale2 = random.randint(0, 255)
                # for index in routes:
                ...
                for ip, pp in enumerate(top):
                    if ip == 0:
                        ...
                        color = (0, 0, 255)
                    cv2.circle(img, (int(pp[0]), int(pp[1])), 3, color, -1)
                ...
                # draw boxes
                for box in boxes:
                    cv2.drawContours(img, [np.array(box)], -1, (0, scale1, scale2), 2)
        # print("nms time: {}".format(time.time() - t0))
        # # cv2.imshow("", img)
        # # cv2.waitKey(0)
        cv2.imwrite('ctw_box{}.jpg'.format(idx), img)
