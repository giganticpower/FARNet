...
from dataset.dast1500 import DAST1500
from network.loss import TextLoss
from network.transnet import TextNet
from util.augmentation import Augmentation, Augmentation1
...
def save_min_loss_model(model, epoch, lr, optimzer):
    save_dir = 'model/min_loss_model/'
    save_path = os.path.join(save_dir, 'textgraph_{}_min_loss.pth'.format(model.module.backbone_name))
    state_dict = {
        'lr': lr,
        'epoch': epoch,
        'model': model.state_dict() if not cfg.mgpu else model.module.state_dict(),
...
def train(model, train_loader, criterion, scheduler, optimizer, epoch, logger, min_loss):
    ...
    if t_loss < min_loss:
        min_loss = t_loss
        save_min_loss_model(model, epoch, scheduler.get_lr(), optimizer)
...
def main():
    ...
    if cfg.exp_name == 'Totaltext':
        trainset = TotalText(
            ...
            ignore_list=None,
            is_training=True,
            transform=Augmentation1(size=cfg.input_size, mean=cfg.means, std=cfg.stds)
        )
        # valset = TotalText(
        #     data_root='data/total-text-mat',
        #     ignore_list=None,
    ...
    elif cfg.exp_name == 'DAST1500':
        trainset = DAST1500(
            data_root='data/OCR-DAS-DATASET',
            is_training=True,
            transform=Augmentation1(size=cfg.input_size, mean=cfg.means, std=cfg.stds)
        )
        valset = None
    ...
    if cfg.freeze:
        for p in model.module.fpn.parameters():
    ...
    if cfg.optim == "Adam" or cfg.exp_name == 'Synthtext':
        ...
        optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=lr, momentum=moment)
    ...
    for epoch in range(cfg.start_epoch, cfg.start_epoch + cfg.max_epoch+1):
        scheduler.step()
        min_loss = train(model, train_loader, criterion, scheduler, optimizer, epoch, logger, min_loss)
