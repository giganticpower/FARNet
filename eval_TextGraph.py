...
from dataset.TD500.eval.msra.eval import test_TD_mean
...
def inference(detector, test_loader, output_dir):
    ...
    for i, (image, train_mask, tr_mask, tcl_mask, radius_map, sin_map, cos_map, meta) in enumerate(test_loader):
        ...
        fps = (i + 1) / total_time
        print('detect {} / {} images: {}. ({:.2f} fps total_time:{:.2f} single_fps:{})'.format(i + 1, len(test_loader), meta['image_id'][idx], fps, total_time,single_fps))
        ...
        if cfg.exp_name == "Icdar2015":
            ...
            # data_transfer_TD500(contours, os.path.join(out_dir, fname))
            data_transfer_MLT2017(contours, os.path.join(out_dir, fname))
...
def main(vis_dir_path):
    ...
    model_path = os.path.join(cfg.save_dir, cfg.exp_name,
                              'textgraph_{}_{}.pth'.format(model.module.backbone_name, cfg.checkepoch))
    ...
    TD_gt_dir = '/data/chh/DRRG-master/dataset/TD500/eval/msra/tdtt/test/'
    inference(detector, test_loader, output_dir)
    TD_dir = os.path.join(output_dir, str(cfg.checkepoch))
    ...
    if cfg.exp_name == "Totaltext":
        ...
        test_TD_mean(TD_dir, TD_gt_dir, cfg.checkepoch)
        # deal_eval_TD500(debug=True)

    # elif cfg.exp_name == "MLT2017":
    #     deal_eval_MLT2017(debug=True)
