...
class KnnGraph(object):
    ...
    def localIPS(self, knn_graph, labels_gt=None):
        ...
        for index, cluster in enumerate(knn_graph):
            ...
            h0 = set(knn_graph[center_idx][1:])  # set() del the repeat
            hops.append(h0)
            ...
            nodes_list.insert(0, center_idx)   # list.insert (index, value) 插入value到列表指定位置
    ...
    def __call__(self, feats, gt_data=None):
        ...
        for bind in range(gt_data.shape[0]):
            ...
            # graphSAGE
            feat_map1 = torch.zeros((node_feat.shape[0], 2*node_feat.shape[1])).cuda()
            for idx, cluster in enumerate(knn_graph):
                feats1 = list()
                center_idx = idx
                feats1.append(knn_graph[center_idx][1:5])
                k = sum(node_feat[torch.tensor(feats1, dtype=torch.long)])
                feat_sum = k[0, :] + k[1, :] + k[2, :] + k[3, :]
                feat_center = node_feat[torch.tensor(center_idx, dtype=torch.long)]
