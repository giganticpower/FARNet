...
class Graph_RPN(object):
    def __init__(self, pooling, pst_dim):
        ...
        # self.clip = (4, 8)  # character wide
        self.clip = (4, 10)  # character wide
    ...
    def graph_generate(self, knn_graph, feat_map):
        ...
        # graphSAGE
        feat_map1 = torch.zeros((feat_map.shape[0], 2*feat_map.shape[1])).cuda()
        # feat_map1 = torch.zeros((feat_map.shape[0], feat_map.shape[1])).cuda()  # not cat
        for index, cluster in enumerate(knn_graph):
            feats = list()
            center_idx = index
            feats.append(knn_graph[center_idx][1:5])
            k = sum(feat_map[torch.tensor(feats, dtype=torch.long)])
            feat_sum = k[0,:] + k[1,:] + k[2,:] + k[3,:]
            feat_center = feat_map[torch.tensor(center_idx, dtype=torch.long)]
            # feat_sum = feat_sum.data.cpu().numpy()
            feat_map1[center_idx] = torch.cat((feat_sum, feat_center), dim=0)
            # feat_map1[center_idx] = 0.1*feat_sum + feat_center
            # feat_map1[center_idx] = feat_sum
        ...
        for index, cluster in enumerate(knn_graph):
            ...
            #     union = len(list(set(ips).union(set(nodes_list))))  # set .union 并集
            #     intersection = len(list(set(ips).intersection(set(nodes_list))))  # set .intersection 交集
        ...
        for idx, ips in enumerate(hops_list):
            ...
            center_feat = feat_map1[torch.tensor(center_idx, dtype=torch.long)]
            feat = feat_map1[torch.tensor(ips, dtype=torch.long)] - center_feat
    ...
    def __call__(self, image, output, graph_feat):
        ...
        # similarity_matrix = np.exp(-similarity_c)*similarity_e
